...
```json
{
  ...
  "translations": {
    "zh-rCN": {
      "name": "琅琊榜",
      "synopsis": "豪门复仇 ...",
      "poster_url": "https://photobundle.oss-ap-southeast-1.aliyuncs.com/Drama/prod/ly/poster/zh-rCN.jpg"
    },
    "en": {
      "name": "Langya Bang",
      "synopsis": null,
  ...
  "subtitles": [
    {
      "lang_code": "en",
      "label": "English",
      "url": "https://photobundle.oss-ap-southeast-1.aliyuncs.com/Drama/prod/ly/ep-3/subtitles/en.vtt"
    }
  ]
}
```
...
HLS 端在收到 2xx 后会调用 `unpublish_episode_from_prod(slug, ep_dir)` 一次（前缀级 sweep）把 OSS prod 端该集的全部对象（三档 ladder + cover + 字幕）删掉。
...
- **绝对**（含 `https://photobundle.oss-ap-southeast-1.aliyuncs.com/Drama/prod/...`），不是相对路径。
- **opaque**：业务服务器存进 DB / 透传给 SDK 客户端就行；**MUST NOT 主动 GET** 这些 URL。
- 客户端按 m3u8 / `EpisodeInfo` 里的绝对 URL 直连 OSS（CDN 友好）。

**业务服务器侧不需要任何 HTTP 出站**到 HLS 服务器或到 OSS。整个 `/sync/*` 流程是"接收 JSON → 写 DB → 200"。
...
- HLS 服务器 → 业务服务器：HTTP `/sync/*` 调用。
- 业务服务器 → HLS 服务器：**0 出站**（v2.0 起业务服务器不再拉任何字节）。
- 业务服务器 → 阿里云 OSS：**0 出站**（除非你们想做 prod 切片镜像 / CDN 主动预热，那是优化场景）。
- 客户端 → 业务服务器：SDK API + DRM key（16 字节）。
- 客户端 → 阿里云 OSS：m3u8 / cover / poster / subtitle / init / segment 全部直连 OSS prod 前缀（`Drama/prod/...`），可经 CDN。

整体上业务服务器只承担"元数据 + DRM key"职责；所有大字节资源都不经过它。这意味着业务服务器可以无状态地横向扩展，HLS 服务器可以纯内网部署（业务服务器无需访问它）。
...
| `BIZ_OUT_DIR` | 落盘根目录（m3u8 / DRM key 文件；v2.0 起不再含 cover / poster / subtitle —— 那三类直走 OSS） |
| `BIZ_DB_PATH` | 业务侧 DB（持久化 dramas / episodes 行 + 三类资产的 OSS 绝对 URL） |
...
操作员                HLS 服务器              业务服务器               OSS
  ...
  │                      │                        │                    │
  │                      │ POST /sync/episodes    │                    │
  │                      │   (含 prod m3u8 文本 + prod cover/字幕 URL) │
  │                      ├───────────────────────►│                    │
  │                      │   200 OK (业务服务器 0 出站)                │
  │                      │◄───────────────────────┤                    │
...
客户端                                          业务服务器              OSS
  ...
  ├───────────────────────────────────────────────►│                    │
  │   EpisodeInfo (含 playUrl + 绝对 OSS cover/poster/subtitle URL)    │
  │◄───────────────────────────────────────────────┤                    │
  │                                                │                    │
  │ GET m3u8                                       │                    │
  ├───────────────────────────────────────────────►│                    │
  ...
  ├───────────────────────────────────────────────►│                    │
  │                                                │                    │
  │ GET cover.jpg / poster.jpg / subtitle.vtt      │                    │
  ├──────────────────────────────────────────────────────────────────► │ (Drama/prod/)
  │ GET init-720p.mp4                              │                    │
  ├──────────────────────────────────────────────────────────────────► │
...
1. 把 `/sync/*` 处理流程里的"GET poster_url / cover_url / subtitle_url 拉字节"步骤删掉。
2. 把 `poster_url` / `cover_url` / `subtitles[].url` 当 opaque 字符串存进 DB；`EpisodeInfo` / `DramaSummary` 输出时 verbatim 透传。
3. 删除业务服务器侧的 `HLS_STAGING_HOST` env / 配置。
4. 删除 v1.0 在 `<biz_OUT_DIR>/{slug}/poster/`、`<biz_OUT_DIR>/{slug}/{ep_dir}/cover.jpg`、`<biz_OUT_DIR>/{slug}/{ep_dir}/subtitles/` 留下的本地文件（迁移完后业务服务器不再拥有这些字节，HLS 端 + OSS 是唯一来源）。
5. 502 异常分支 / 错误处理代码可以删了。

HLS 端配套：升级前如果已经有线上数据，需要跑一次 `OSS_ENABLED=true ./venv/bin/python scripts/backfill_assets_to_oss.py` 把现存的本地海报/封面/字幕回填到 OSS staging，然后操作员点"[同步全部]"把 prod 端推上去。

...
| `POST /admin/dramas/{slug}/episodes/batch-nas` | nas-source-ingest：扫一个 NAS 文件夹里的 `EP<n>.<ext>` 逐集**原地编码**（不上传）。body `{dir}` 相对根。已存在的集走重传语义覆盖、`status=encoding` 跳过。返回 `{ok_count, error_count, results[], dir}`。503 / 404 / 400。 |

...
| `SOURCE_NAS_DIR` | no | `/Volumes/酷讯短剧组` | 共享 NAS 片源根目录（部署机可直接读取）。**nas-source-ingest**：设置且当前是可访问目录时启用——运营在 `/admin` 直接从 NAS 选片源编码，**无需经浏览器再上传一次**。pipeline **原地读取**该文件，worker 编码后**保留**（只有 `UPLOAD_TMP_DIR` 下的文件才在成功后删——keep/delete 策略按路径位置判定，见 `work_queue.is_tmp_source`）。所有操作员选择的路径都被 `app/nas.py` 限制在此根内（`resolve()` 后校验仍在根下，拒绝 `../` 与越界软链）。设为空（`SOURCE_NAS_DIR=`）禁用：NAS 按钮隐藏、`/admin/nas/*` 与 `/episodes/{nas,batch-nas}` 返回 503。`nas_import_enabled` 据 `is_dir()` 判定（瞬时未挂载即禁用，而非深处报错）。**Docker 部署须把 NAS 挂载点 bind-mount 进容器同一路径**（见 `docker-compose.yml` 注释）。 |
...
| `AI_TRANSLATE_API_KEY` | no | _unset_ | AI 翻译总开关（kie.ai 风格 OpenAI 兼容 chat-completions，Bearer token）。**未设时功能整体禁用**：`POST /admin/{dramas,tags,actors}/.../translate` 与 `.../subtitles/translate` 返回 503，「AI 翻译」按钮 + 导航「翻译任务」隐藏（`settings.ai_translate_enabled` 据此判定）。设了即把剧名/简介、标签 label、演员 name、字幕从各自 `default_lang`/源字幕翻译并**直接覆盖**到其余全部已注册语言。**走后台任务队列 `ai-translation-queue`**（`app/ai_jobs.py`）：端点只入队 + 返回 202，worker 池逐语言执行，落库走现有 `upsert_*_translation` / 字幕写盘 + `mark_*_dirty`，天然进入 staging→手动同步流。粒度=每目标语言一个 `translation_jobs` 行；**一键翻译是覆盖式**——每次都重翻全部其它已注册语言，**不按 job 历史跳过**（历史会在译文被手动删改后过期，跳过会漏翻）；入队前清掉这些语言的旧 terminal job 记录（保证进度准确 + 表不无限增长）。重启 reap `running`→`queued`，dedupe 唯一索引防重复 in-flight（双击时已在队列的语言计入 `skipped`）。**这是除业务同步外唯一的公网出站**（kie.ai）。HTTP 层 `app/ai_translate_client.py`（mirror `sync_client.py`），端点 `app/routers/ai_translate.py`，VTT 解析 `app/vtt.py`，进度页 `/admin/translations`。 |
...
| `AI_TRANSLATE_TIMEOUT` | no | `120` | 单次翻译 HTTP 请求超时（秒）。整数；非正值 → 启动期 fail-fast。 |
| `AI_TRANSLATE_CONCURRENCY` | no | `2` | 后台翻译 worker 并发数（同时跑几个语言任务）。控 kie.ai 限流 / 成本，配额宽裕再调高。整数 `>= 1`；非法值 → 启动期 fail-fast。 |
...
| `GET /admin/nas/browse` | nas-source-ingest：列出 `SOURCE_NAS_DIR` 下某目录（`?path=` 相对根，''=根），供页面 NAS 浏览器。返回 `{path, parent, dirs[], files[]}`（仅 video 扩展名文件、隐藏 dotfile 与越界软链）。503 未启用 / 400 越界。 |
| `POST /admin/dramas/{slug}/episodes/nas` | nas-source-ingest：把 NAS 上单个文件作下一集**原地编码**（不上传）。body `{path}` 相对根。自增集号 + 并发冲突重试 3×（同 upload 路由）。**声明在 `episodes/{ep}` 之前**。503 / 404 / 400 / 200 `{ep_number}`。 |
| `POST /admin/dramas/{slug}/episodes/batch-nas` | nas-source-ingest：把**多选**的一批 NAS 文件逐个**原地编码**（不上传）。body `{paths}` = 相对根的文件路径数组（运营在浏览器里多选）；每个文件集号取自 `EP<n>` 文件名前缀。已存在的集走重传语义覆盖、`status=encoding` 跳过。返回 `{ok_count, error_count, results[]}`。503 / 404 / 400。 |
| `POST /admin/dramas/{slug}/episodes/{ep}/nas` | nas-source-ingest：用 NAS 文件给某集**换源重编**（新版本，原地读）。body `{path}` 相对根。503 / 404 / 409（encoding）/ 400 / 200 `{ep_number, upload_version}`。 |
...
| `POST /admin/actors/translate` | AI **库级**演员翻译入队（演员库「翻译所有演员」/「补全演员翻译」按钮，免去逐演员点开）：`?mode=overwrite\|missing`。遍历所有演员，对**有默认语言 name** 的演员扇出每目标语言一个 job；`overwrite` 覆盖其它全部语言，`missing` 只补该演员还没有的语言。无默认 name 的演员跳过。进度由 `kind_translation_progress('actor')` 跨全部演员聚合（前端 `pollTranslation('actor', null, null)`）。**路由声明在 `/{slug}/translate` 之前**，否则 `translate` 字面段会被 `{slug}` 捕获。503 / 200 `{noop, actors_without_source}` / 202 `{enqueued, actors_translated, actors_without_source, skipped, mode}`。 |
...
**nas-source-ingest（直接读 NAS 片源，免上传）**：内网部署机与运营共享一个 NAS 挂载（`SOURCE_NAS_DIR`，默认 `/Volumes/酷讯短剧组`）。运营点开 `/admin` 里的 NAS 浏览器（`app/static/admin.js` 的 `window.openNasBrowser`，模态框在 `_base.html`，列目录走 `GET /admin/nas/browse`）选文件/文件夹，服务器**原地**把该路径当 pipeline 源编码——不再经浏览器重传一遍。三个入口对应三种粒度：剧详情页「NAS 下一集」(单文件→自增集)、「NAS 批量」(列表多选文件→逐个按 `EP<n>` 文件名入队)、集详情页「从 NAS 换源」(单文件→该集新版本)。上传路与 NAS 路共用同一套 ingest 核心 `_ingest_episode_from_file`（ffprobe + 封面 + `upsert_pending`）；上传路先把字节落到 `UPLOAD_TMP_DIR` 再调它，NAS 路直接传 NAS 绝对路径。**keep/delete 策略按源文件位置判定**（`work_queue.is_tmp_source`：`UPLOAD_TMP_DIR` 下 = 临时拷贝，编码成功后删；其它如 NAS = 运营 source-of-truth，**永不删**，只清 `source_path` 列）。重试 / 重传 / 替换旧版本时清理旧源也走同一判定，绝不误删 NAS 文件。路径安全：`app/nas.py` 把操作员路径 `resolve()` 后校验仍在根下，拒绝 `../` 与越界软链；只列 video 扩展名、隐藏 dotfile。Docker 部署须把 NAS bind-mount 进容器同一路径（`docker-compose.yml` 注释）。

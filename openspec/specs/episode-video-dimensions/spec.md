# episode-video-dimensions

上传时 ffprobe 探测源视频 codec width / height；DB 存两列；`EpisodeInfo` 透传给 SDK；老行兼容 null。归档自 `2026-04-30-episode-video-dimensions`。

> 实施补注：归档后通过 `scripts/backfill_video_dimensions.py` 一次性回填了老 ready 行 —— 用 ffprobe 读 `init-720p.mp4` 的 codec dimension（aspect ratio 守恒；绝对值是编码后 720p 的尺寸）。新上传仍走 `probe_video_dimensions(源)` 拿源 dimension。两路口径轻微不一致，aspect 一致；接受这个 trade-off。

...
4. **Inline editor panels / fields**: same as before, plus drama-level business fields for `free_episodes` and `is_ongoing` ("连载中" / "已完结"). After save, the drama's sync_status flips to `dirty` (per the dirty-marking rules in `business-server-sync`); the page refreshes to reflect the new state.

...
After step 5 (`oss-staging-prod-separation`) and step 6 (`business-server-sync`), the object bucket is the source of truth for **video** assets. The system originally wrote staging and copied to prod during sync; after the online migration, upload/encode paths write prod directly and sync only publishes prod object keys / playlist text.
...
That contract has known weaknesses (network coupling, 502 failure path in protocol, CDN unfriendly, role smearing). This change unifies all asset families under the same direct-prod pattern that videos now use.
...
- Prefix `unpublish_*_from_*` helpers continue to work without modification — they delete by **prefix**, so `Drama/prod/{slug}/poster/` etc. is naturally swept along.
...
- m3u8 files: do NOT live on OSS/TOS at all. The local playlist remains under `OUT_DIR`; the prod-flavored playlist is shipped inline as a string in the `/sync/episodes` payload and stored on the business server's filesystem.
...
We add per-asset helpers that each know their on-disk source path and prod object key:

```python
def upload_poster_to_staging(slug: str, lang: str, local_path: Path) -> str:
    """Historical name; upload OUT_DIR/{slug}/poster/{lang}.{ext}
    → Drama/prod/{slug}/poster/{lang}.{ext}. Called from POST /admin/dramas/{slug}/poster.
    """

def publish_poster_to_prod(slug: str, lang: str, ext: str) -> str:
    """Return Drama/prod/{slug}/poster/{lang}.{ext}. Called from sync."""
...
3. Uploads to object-storage prod via `bucket.put_object_from_file` (`asyncio.to_thread`-wrapped; SDK is sync)
...
### Decision: sync flow generates prod keys before the HTTP call
...
`publish_*_to_prod` helpers do not list/copy remote objects in the normal sync path. They may still raise for local preconditions such as a missing local playlist needed to build m3u8 text; those failures keep the sync row `sync_failed` and the business call is **not** made. Missing prod objects are checked by `scripts/audit_prod_assets.py` outside the operator sync path.
...
```python
# walk OUT_DIR, upload every cover.jpg / poster/*.* / subtitles/*.vtt to prod object storage
# idempotent (overwrites).
# DOES NOT touch DB.
# Operator runs once after deploy.
```

After backfill, the next sync of each drama / episode will publish the prod keys and update the business server.
...
3. Prod cleanup: `unpublish_poster_from_prod(slug, "xx")` (deletes any extension under prefix `Drama/prod/{slug}/poster/xx.*`)
4. Prod upload: `bucket.put_object_from_file("Drama/prod/{slug}/poster/xx.jpg", local_path)`
...
- **Risk: A prod object is missing even though DB points at it.**
  → Mitigation: upload/encode paths fail fast before rows become ready/dirty; after the migration, operators can run `scripts/audit_prod_assets.py` to compare current DB rows with prod object keys by prefix.
...
4. Run `STORAGE_PROVIDER=tos ./venv/bin/python scripts/backfill_assets_to_oss.py`. This walks every `cover.jpg`, every `poster/{lang}.{ext}`, every `subtitles/{lang}.vtt` under `OUT_DIR/{slug}/...`, filters by DB-row existence, and uploads to prod. Idempotent.
...
7. Run `./venv/bin/python scripts/audit_prod_assets.py` or verify in the OSS/TOS console: `Drama/prod/{slug}/poster/...`, `Drama/prod/{slug}/{ep_dir}/cover.jpg`, `Drama/prod/{slug}/{ep_dir}/subtitles/...` exist.
...
- **Should we also publish posters / covers to prod when operators edit them, even before they click "sync"?** Answer: yes, after migration this is the normal path. Versioned object names avoid polluting already-synced client URLs; sync remains the explicit metadata publish step.

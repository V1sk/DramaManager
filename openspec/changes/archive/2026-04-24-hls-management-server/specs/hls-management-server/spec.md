...
The service SHALL accept an HTTP `POST /admin/upload` request with `multipart/form-data` carrying the fields `video` (required file), `drama_slug` (required text), `drama_name` (required text), and `ep_number` (required integer ≥ 1). It SHALL reject requests whose `drama_slug` does not match `^[a-z0-9][a-z0-9-]*$`, whose `ep_number` is missing or non-positive, whose `drama_name` is empty after trimming, or whose `video` part is missing.

#### Scenario: Valid upload is accepted
- **WHEN** a client posts `video=<mp4>`, `drama_slug=langyabang`, `drama_name=琅琊榜`, `ep_number=3`
...
The service SHALL persist each episode in a SQLite table with the columns `id`, `drama_slug`, `drama_name`, `ep_number`, `episode_id`, `status`, `duration_ms`, `play_url`, `key_uri`, `key_b64`, `iv_hex`, `cover_url`, `source_filename`, `error_message`, `created_at`, `updated_at`. The tuple `(drama_slug, ep_number)` SHALL be unique and `episode_id` SHALL be `"{drama_slug}-ep-{ep_number}"` and itself unique.

#### Scenario: Re-uploading the same slug and episode overwrites
- **GIVEN** a row exists with `drama_slug=langyabang`, `ep_number=3`, `status=ready`
- **WHEN** a new valid upload arrives with the same `drama_slug` and `ep_number`
- **THEN** the existing row is updated in place (`status` reset to `pending`, `updated_at` refreshed, `error_message` cleared)
...
The service SHALL serve `GET /admin` returning an HTML page containing an upload form (fields: drama slug, drama name, episode number, video file) and an episode list. Each list entry SHALL show the cover as a thumbnail, the drama name, episode number, status, `duration_ms` (when present), `created_at`, and — when status is `failed` — the `error_message`. Clicking the cover SHALL open a file picker that POSTs to `/api/episodes/{slug}/{ep}/cover`.

#### Scenario: Admin page loads with current data
- **WHEN** the client requests `GET /admin`
- **THEN** the response is 200 HTML that includes the upload form and a list seeded from `GET /admin/episodes`
